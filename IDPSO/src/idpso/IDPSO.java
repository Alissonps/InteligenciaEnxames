...
public class IDPSO {
	...
	public IDPSO(int iteracoes, int n_particulas, double inercia_inicial,
			double inercia_final, double n1, double n2, int equacao,
			int s_mudanca) {
	...
	public void Fitness() {
		...
		for (int i = 0; i < this.particulas.length; i++) {
			...
			if (this.equacao == 0) {
				this.fitness[i] = Math.abs(Sphere_Fuction(particulas[i])
						- this.minimo);
			} else if (this.equacao == 1) {
				this.fitness[i] = Math.abs(Schwefels_Fuction(particulas[i])
						- this.minimo);
			} else if (this.equacao == 2) {
				this.fitness[i] = Math.abs(Rosenbrock_function(particulas[i])
						- this.minimo);
			} else if (this.equacao == 3) {
				this.fitness[i] = Math.abs(Rastrigins_function(particulas[i])
						- this.minimo);
			} else if (this.equacao == 4) {
				this.fitness[i] = Math.abs(Scaffers_function(particulas[i])
						- this.minimo);
			}
	...
	public void Atualizar_Parametros(int interação) {
		...
		for (int i = 0; i < this.particulas.length; i++) {
			for (int j = 0; j < numero_Dimensoes; j++) {
				...
				// gbest e pbest
				if ((this.gBest[j] == this.particulas[i][j])) {
				...
				if (this.pBest[i][j] == this.particulas[i][j]) {

					parte2 = this.pBest[i][j];
		...
		for (int i = 0; i < this.particulas.length; i++) {
			...
			double calculo = this.phi[i]
					* (interação - ((1 + ln) * this.iteracoes) / this.mi);

			this.inercia[i] = ((this.inercia_Inicial - this.inercia_Final) / (1 + Math
					.exp(calculo))) + this.inercia_Final;
	...
	public void Atualizar_Particulas() {
		...
		for (int i = 0; i < this.particulas.length; i++) {
			for (int j = 0; j < numero_Dimensoes; j++) {
				...
				this.particulas[i][j] = this.particulas[i][j]
						+ this.velocidade[i][j];
	...
	public double Schwefels_Fuction(double[] particulas) {
		...
		for (int j = 0; j < this.particulas[0].length; j++) {
			...
			soma = soma + particulas[j]
					* Math.sin(Math.sqrt(Math.abs(particulas[j])));
	...
	public double Rosenbrock_function(double[] particulas) {
		...
		for (int j = 0; j < this.particulas[0].length - 1; j++) {
			...
			soma = soma
					+ (100 * (Math.pow((1 - Math.pow(x, 2)), 2)) + Math.pow(
							(1 - 1), 2));
		...
		for (int j = 0; j < this.particulas[0].length - 1; j++) {
			...
			soma = soma
					+ (100 * (Math.pow((xnext - Math.pow(x, 2)), 2)) + Math
							.pow((x - 1), 2));
	...
	public double Rastrigins_function(double[] particulas) {
		...
		for (int j = 0; j < this.particulas[0].length - 1; j++) {
			...
			soma = soma
					+ (Math.pow(particulas[j], 2)
							- (10 * Math.cos(2 * Math.PI * particulas[j])) + 10);
	...
	public void Executar() throws InterruptedException {
		...
		while (i < iteracoes) {
			...
			System.out.println(i + "- Melhor Fitness da iteração: " + fitBest
					+ "\n");
			System.out.println("\n   Inercia da particula 0: "
					+ this.inercia[0] + "\n");

			/*
			 * // plots da otimização das particulas Plot2DPanel plot_particulas
			 * = new Plot2DPanel(); plot_particulas.addScatterPlot("Particulas",
			 * this.particulas);
			...
			/*
			 * // plots da otimização do fitness Plot2DPanel plot_fitness = new
			 * Plot2DPanel(); plot_fitness.addLinePlot("Fitness", grafico);
	...
	public static void main(String[] args) throws InterruptedException {
		...
		IDPSO p1 = new IDPSO(100000, 20, 0.9, 0.4, 1.4, 1.4, 4, 1000);
		p1.Executar();
